...
export default function ABTestingSimulator() {
  ...
  return (
    <div className="w-full max-w-5xl mx-auto flex flex-col gap-10">
      ...
      {/* Mode Selectors */}
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 border-b border-black/[0.05] pb-6">
        <div>
        ...
        {/* Tab Controls */}
        <div className="bg-bg-secondary p-1.5 rounded-full border border-black/[0.05] flex gap-2 w-full sm:w-auto">
          <button
            onClick={() => {
              setActiveTab("before");
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-stretch">
        ...
        <div className="lg:col-span-7 flex justify-center items-center">
          <div className="relative w-full max-w-sm rounded-[32px] bg-neutral-900 border-4 border-neutral-800 p-4 pt-8 shadow-2xl overflow-hidden aspect-[9/18]">
            ...
            <div className="w-full h-full rounded-2xl bg-white border border-black/[0.05] p-4 flex flex-col justify-between overflow-y-auto relative no-scrollbar">
              
              {/* Product Header */}
              <div className="flex justify-between items-center pb-2.5 border-b border-black/[0.05] mb-3">
                <span className="text-[10px] text-[#7a7a7a] font-mono tracking-widest">STOREFRONT</span>
                <span className={`text-[10px] font-bold font-mono px-2 py-0.5 rounded ${
                  activeTab === "before" ? "bg-red-500/10 text-red-600" : "bg-primary/10 text-primary"
                }`}>
              ...
              <div className="flex flex-col gap-4">
                {/* Product Image Panel */}
                <div className="relative aspect-[4/3] rounded-xl bg-gradient-to-tr from-bg-secondary to-white border border-black/[0.05] flex items-center justify-center overflow-hidden">
                  ...
                  <span className="absolute bottom-2 left-2 text-[9px] text-[#4a4a4a] bg-bg-secondary/70 px-2 py-0.5 rounded backdrop-blur">
                    Mockup Device Case
                  </span>
                </div>
                ...
                <div className="flex flex-col gap-1.5">
                  ...
                  <h4 className="text-sm font-bold text-[#1a1a1a]">Classic Leather Case v2</h4>
                  <div className="flex gap-2 items-baseline">
                    <span className="text-base font-extrabold text-[#1a1a1a]">₹1,499</span>
                    {activeTab === "after" && (
                      <span className="text-[10px] text-red-500 line-through">₹2,499</span>
                    )}
                  </div>
                </div>
                ...
                {activeTab === "before" ? (
                  <div className="flex flex-col gap-2.5 text-[10px] text-[#4a4a4a] bg-bg-secondary/50 p-3 rounded-lg border border-black/[0.05]">
                    <p className="leading-relaxed">
                      A regular, unoptimized product layout. Huge hero graphics trigger a high loading delay.
                    </p>
                    <p className="text-[9px] text-red-500/80">✗ Customer rating score hidden below fold</p>
                    <p className="text-[9px] text-red-500/80">✗ Cluttered options selectors</p>
                    <p className="text-[9px] text-red-500/80">✗ No mobile checkout shortcuts</p>
                  </div>
                ) : (
                  <div className="flex flex-col gap-3">
                    ...
                    <div className="flex flex-col gap-1.5">
                      <div className="flex justify-between text-[10px] text-[#4a4a4a]">
                        ...
                        <span className="text-[#1a1a1a] font-bold">Midnight Black</span>
                      </div>
                      <div className="flex gap-2">
                        <span className="text-[9px] border border-primary text-primary bg-primary/5 px-3 py-1 rounded-md font-bold">
                          Black
                        </span>
                        <span className="text-[9px] border border-black/[0.08] text-[#7a7a7a] px-3 py-1 rounded-md">
                          Tan Brown
                        </span>
                        <span className="text-[9px] border border-black/[0.08] text-[#7a7a7a] px-3 py-1 rounded-md">
                          Olive Green
                    ...
                    {/* Trust Indicators */}
                    <div className="grid grid-cols-2 gap-2 text-[8px] font-mono text-[#1a1a1a]">
                      <div className="flex items-center gap-1 bg-bg-secondary p-1.5 rounded border border-black/[0.05]">
              ...
              <div className="mt-4 pt-3.5 border-t border-black/[0.05] flex flex-col gap-2">
                <button className={`w-full py-2.5 rounded-lg text-xs font-bold uppercase tracking-wider flex items-center justify-center gap-2 ${
                  activeTab === "before"
                    ...
                    : "bg-primary text-white hover:bg-[#2a6350] shadow-[0_8px_24px_rgba(55,126,98,0.25)]"
                }`}>
                  {activeTab === "before" ? (
                ...
                {activeTab === "after" && (
                  <p className="text-[8px] text-[#4a4a4a] text-center">
                  ...
                  {HOTSPOTS.map((spot) => (
                    <button
                      ...
                      style={{
                        top: spot.y,
                        left: spot.x,
                        boxShadow: hoveredHotspot === spot.id ? "0 0 15px #377e62" : "0 0 10px rgba(0,0,0,0.3)"
                      }}
                      title={spot.title}
                    >
                      !
        ...
        <div className="lg:col-span-5 flex flex-col justify-between p-6 sm:p-8 rounded-2xl bg-white border border-black/[0.05] shadow-sm">
          <div className="flex flex-col gap-6">
            <span className="text-xs text-[#4a4a4a] font-mono uppercase tracking-widest">Auditing Optimization Parameters</span>
            <h4 className="text-lg font-bold text-[#1a1a1a]">
              {activeTab === "before" ? "Why Cluttered Layouts Fail" : "How Optimization Multiplies Sales"}
            </h4>

            {activeTab === "before" ? (
              <div className="flex flex-col gap-4 text-xs sm:text-sm text-[#4a4a4a] leading-relaxed">
                <p>
            ...
            {activeTab === "after" && (
              <div className="min-h-[140px] mt-4 p-4 rounded-xl bg-bg-secondary border border-black/[0.05] transition-all flex flex-col justify-center">
                <AnimatePresence mode="wait">
                  {hoveredHotspot !== null ? (
                    ...
                    <motion.div
                      initial={{ opacity: 0 }}
                      animate={{ opacity: 1 }}
                      className="text-center py-4 text-xs text-[#4a4a4a] flex flex-col items-center gap-2"
                    >
          ...
          <div className="mt-8 pt-4 border-t border-black/[0.05] flex flex-col gap-2">
            <div className="flex justify-between items-center text-xs font-mono">
              <span className="text-[#4a4a4a]">Optimization Focus:</span>
              <span className="text-[#1a1a1a] font-bold">Mobile First</span>
            </div>
          </div>
        </div>
      </div>
